...
```
.ai/story-agent/           ← copied into every target repo on init
...
| Hard rules (no fabrication, no paraphrasing ACs, stop at plan.md) | Inline in each agent file under `.ai/story-agent/agents/` |
...
| GitHub Copilot agent wrappers (`.github/agents/`) | `.ai/story-agent/templates/github/agents/` |
| Cursor skill wrappers (`.cursor/skills/`) | `.ai/story-agent/templates/cursor/skills/` |
...
- If a file path or field is not in the workspace or fetched data, mark it "candidate, verify".
- Copy acceptance criteria verbatim from the tracker. Never paraphrase.
...
- A hard constraint is added or removed
