...
## Source of truth
- Agent definitions: `.ai/agents/`. When the user types a phrase matching an agent's trigger, follow that agent's workflow verbatim.
- Analysis lenses: `.ai/prompts/`. Apply on request, e.g. "apply rollback-risks to this PR".
...
- **No secrets in outputs.** Tokens, PATs, connection strings never appear in `.ai/outputs/` or chat.
